...
<div class="container">
    <form method="POST" action="{{ route('admin.plates.update', $plate->id) }}" enctype="multipart/form-data"
        ...
        <div class="mb-3">
            ...
            <input id="name" type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name',$plate->name) }}">
            @error('name')
        ...
        <div class="mb-3">
            ...
            <select class="form-control" name="category_id" id="category_id">
                ...
                @foreach ($categories as $category)
                    <option value="{{ $category->id }}" @if ($category->id == old('category_id', $category->id)) selected @endif>
                        {{ $category->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input id="price" type="text" class="form-control @error('price') is-invalid @enderror" name="price" value="{{old('price',$plate->price)}}">
            @error('price')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
        ...
        <div class="mb-3">
            <div class="form-check">
                <input type="hidden" name="visible" class="switch-input" value="0" {{ old('visible', $plate->cover) ? 'checked="checked"' : '' }} />
                <input type="checkbox" name="visible" class="switch-input" value="1" {{ old('visible', $plate->visible) ? 'checked="checked"' : '' }} />
